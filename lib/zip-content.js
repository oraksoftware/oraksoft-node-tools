...
export async function deployZipContent() {
  ...
  const filesToArchive = config["zip-content"];

  // Debug
  if (!filesToArchive) {
    console.error("Error: 'zip-content' alanı orak-config.json içinde tanımlanmalıdır.");
    process.exit(1);
  }
  ...
  const distDir = path.resolve(projectRoot, '.orak-dist');
